...
contains
  ...
  subroutine scanline_fracture(trackers, fractured_boxes)
    ...
    call CancelTrackers( trackers, n )
    !n = size(trackers)
  ...
  subroutine heal_boxes(input_box_count, boxes, output_box_count)
    ...
    integer :: i
    integer(kind=int64) :: n, output_tracker_count
    ...
    !call PerformCancellation(events)
    !call CancelTrackers( events, output_tracker_count )
  ...
  subroutine CancelTrackers(trackers, output_count)
    ! Assuming type XYTracker is available in the module/interface scope
    type(XYTracker), dimension(:), intent(inout) :: trackers
    integer(kind=int64), intent(out)                         :: output_count

    integer :: i
    logical :: is_coincident
    logical :: opposite_winding

    output_count = 0

    ! Handle empty array edge-case safely
    if (size(trackers) == 0) return

    do i = 1, size(trackers)
       if (output_count > 0) then
          ! 1. Check if coordinates perfectly match the top of our stack
          is_coincident = (trackers(i)%X == trackers(output_count)%X) .and. &
               (trackers(i)%Y == trackers(output_count)%Y)

          if (is_coincident) then
             ! 2. Check for opposite winding (opposite signs)
             !    We explicitly check > 0 and < 0 assuming 0 is not a valid polygon ID.
             opposite_winding = &
                  (trackers(i)%polygonNumber > 0_int8 .and. trackers(output_count)%polygonNumber < 0_int8) .or. &
                  (trackers(i)%polygonNumber < 0_int8 .and. trackers(output_count)%polygonNumber > 0_int8)

             if (opposite_winding) then
                ! 3. Cancel out by "popping" the stack and skipping to the next tracker
                output_count = output_count - 1
                cycle
             end if
          end if
       ...
       ! 4. If no cancellation occurred, "push" the tracker to the stack
       output_count = output_count + 1

       ! Optimization: Only copy memory if the read and write heads have diverged
       if (i > output_count) then
          trackers(output_count) = trackers(i)
       end if

    end do

  end subroutine CancelTrackers
