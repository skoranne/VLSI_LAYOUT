...
contains
  ...
  subroutine SimpleWindow(input_layer, WIN_X, WIN_Y)
    ...
    do while (glfwWindowShouldClose(window) == 0)
       ...
       if (glfwGetKey(window, GLFW_KEY_EQUAL) == GLFW_PRESS) zoom  = zoom / 1.02_c_double ! '-' key (Zoom Out)
       ! --- QUIT ON ESCAPE ---
...
program main
  ...
  implicit none
  integer :: narg, iostat
  integer(kind=int64) :: N, total_nodes
  type(Layer),target     :: input_layer
  character(len=256)     :: filename
  narg = command_argument_count()
  if( narg /= 1 ) error stop "./MAGVIEW KLBIN"
