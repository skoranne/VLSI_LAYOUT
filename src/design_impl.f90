...
contains
   ...
   module subroutine CalculateFrameNOT( input_layer_A, input_layer_B, output_layer )
      ...
      integer(kind=int64) :: number_leaves, total_count, i
      type(Box) :: bboxA
      type(Layer) :: temp_layer
      integer :: nthreads, tid, alloc_status
      integer(kind=int64), parameter :: K_INIT_BOX_COUNT = 4096
      integer(kind=int64), parameter :: K_GRID = 2
      integer :: rows, cols
      type(Layer) :: gridA
      ...
      call CreateGrid( input_layer_A, gridA, K_GRID, K_GRID)
      ...
      bboxA = mbr_of_array( input_layer_A%layer_boxes, input_layer_A%n_used )
      do rows = 1, K_GRID
         write(*,*) 'Processing ROW: ', rows
         do cols = 1, K_GRID
            write(*,*) 'Processing ', rows, ' ', cols, ' Flat address ', (rows-1)*K_GRID+cols
            grids(rows,cols) = gridA%layer_boxes( (rows-1)*K_GRID+cols )
         end do
      end do
      do rows = 1, K_GRID
         do cols = 1, K_GRID
            !> find all B inside this GRID
            call generate_trackers( input_layer_B%layer_boxes, grids(rows,cols)%layer_boxes(1), &
            ...
            call CalculateAND( input_layer_A, templayerGrid(rows,cols), outputGrid(rows,cols))
