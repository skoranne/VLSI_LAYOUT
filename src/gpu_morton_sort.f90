...
module MortonSortOMT
   ...
   public:: SortBoxesDirect, SortBoxesIndirect

   ! ---------------------------------------------------------------------------
   ! C/C++ Interface Bindings to Thrust Wrappers
   ! ---------------------------------------------------------------------------
   interface
      subroutine thrust_sort_boxes_direct(d_boxes, n) bind(c, name="thrust_sort_boxes_direct")
         import :: c_ptr, c_int64_t
         type(c_ptr), value :: d_boxes
         integer(c_int64_t), value :: n
      end subroutine thrust_sort_boxes_direct

      subroutine thrust_sort_boxes_indirect(d_boxes, d_indices, n) bind(c, name="thrust_sort_boxes_indirect")
         import :: c_ptr, c_int64_t
         type(c_ptr), value :: d_boxes
         type(c_ptr), value :: d_indices
         integer(c_int64_t), value :: n
      end subroutine thrust_sort_boxes_indirect
   end interface
...
contains
   ...
   subroutine SortBoxesDirect(Boxes, N)
      type(Box), intent(inout), target :: Boxes(:)
      integer(kind=int64), intent(in) :: N

      if (N <= 1) return

      ! Map the array to the device (or use it if already there)
      ! 'use_device_ptr' extracts the raw device address to pass to C++
      !$omp target data map(tofrom: Boxes(1:N)) use_device_ptr(Boxes)

      call thrust_sort_boxes_direct(c_loc(Boxes(1)), N)
   ...
   subroutine SortBoxesIndirect(Boxes, N, SortedIndices)
      ...
      integer(kind=int64), intent(in) :: N
      integer(kind=int64), intent(out), target :: SortedIndices(:)

      if (N <= 1) return

      ! Boxes are read-only (map 'to').
      ! SortedIndices is allocated/written on GPU and copied back (map 'from').
      !$omp target data map(to: Boxes(1:N)) map(from: SortedIndices(1:N)) &
      !$omp use_device_ptr(Boxes, SortedIndices)

      call thrust_sort_boxes_indirect(c_loc(Boxes(1)), c_loc(SortedIndices(1)), N)
   ...
   !> Direct Sort: Physically sorts the array of Boxes based on Morton Code
   subroutine SortBoxesDirectOld(Boxes, N)
   ...
   end subroutine SortBoxesDirectOld

   !> Indirect Sort: Leaves Boxes untouched, returns an array of sorted indices.
   !> Ideal for keeping massive layout structures in immutable memory.
   subroutine SortBoxesIndirectOld(Boxes, N, SortedIndices)
   ...
   end subroutine SortBoxesIndirectOld
